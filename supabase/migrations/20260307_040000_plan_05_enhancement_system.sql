...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('error', 'İşlem sırasında altın yetersiz kaldı');
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('error', 'Geliştirme parşömeni yetersiz');
  END IF;
  ...
  IF p_rune_type != 'none' THEN
    ...
    UPDATE public.inventory SET quantity = quantity - 1 WHERE row_id = (SELECT row_id FROM target_stack) AND quantity > 0;
    IF NOT FOUND THEN
      RETURN jsonb_build_object('error', 'Seçilen rune yetersiz');
    END IF;
